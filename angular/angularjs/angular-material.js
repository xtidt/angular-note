...
var transforms = (function() {
  // pengchong改造，让粒度更细
  var values = new Array(1001);
  for(var i = 0; i < 1001; i++){
    values[i / 10] = makeTransform(i / 10);
  }
...
function SliderDirective($$rAF, $window, $mdAria, $mdUtil, $mdConstant, $mdTheming, $mdGesture, $parse) {
  ...
  function postLink(scope, element, attr, ngModelCtrl) {
    ...
    function ngModelRender() {
      ...
      // 改造，支持文字提示
      var tips = element.attr('ap-slider-tips');
      thumbText.text(tips ? $parse(tips)()[ngModelCtrl.$viewValue] : ngModelCtrl.$viewValue);
    }
    ...
    function adjustThumbPosition( x ) {
      ...
      // 改造，支持文字提示
      var tips = element.attr('ap-slider-tips');
      thumbText.text(tips ? $parse(tips)()[closestVal] : closestVal);
    }
